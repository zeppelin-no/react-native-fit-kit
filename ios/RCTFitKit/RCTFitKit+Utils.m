...
@implementation RCTFitKit (Utils)
...
+ (NSPredicate *)predicateForSamplesOnDayFromTimestamp:(NSString *)timestamp {
    NSDate *day = [RCTFitKit parseISO8601DateFromString:timestamp];
    return [RCTFitKit predicateForSamplesOnDay:day];
}
...
+ (NSDate *)dateFromOptionsDefaultNow:(NSDictionary *)options {
    NSString *dateString = [options objectForKey:@"date"];
    if(dateString != nil){
        NSDate *date = [RCTFitKit parseISO8601DateFromString:dateString];
        if(date == nil){
